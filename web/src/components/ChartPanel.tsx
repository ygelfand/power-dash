...
import { IconRotateClockwise2, IconChevronLeft, IconChevronRight } from "@tabler/icons-react";
...
export function ChartPanel({
  ...
  const handleResetZoom = (e: React.MouseEvent) => {
    ...
    const duration = parseTimeframe(currentTimeframe);
    const isManualZoom = isZoomed && zoomRange && (Math.abs((zoomRange[1] - zoomRange[0]) - duration) > 10);
              ...
              <Group gap="xs" wrap="nowrap" onClick={(e) => e.stopPropagation()}>
                  {isManualZoom ? (
                      ...
                      <Group gap={0} wrap="nowrap" className={classes.navGroup}>
                          <ActionIcon variant="default" size="sm" onClick={handlePrev} title="Previous" className={classes.navButtonLeft}>
                              <IconChevronLeft size={14} />
                          </ActionIcon>
                          <div className={classes.navPageIndicator}>
                            <Text size="xs" fw={800} className={pageOffset > 0 ? classes.navPageActive : classes.navPageLive}>
                                {pageOffset > 0 ? `-${pageOffset}` : 'LIVE'}
                            </Text>
                          </div>
                          <ActionIcon variant="default" size="sm" onClick={handleNext} disabled={!canGoForward} title="Next" className={classes.navButtonRight}>

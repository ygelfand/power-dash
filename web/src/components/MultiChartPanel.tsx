import { Text, SimpleGrid, Group, ActionIcon, SegmentedControl, Badge } from '@mantine/core';
...
export function MultiChartPanel({ title, charts, onClick, onSelect, cols = 2, timeframe: initialTimeframe, tooltipFormat, height }: MultiChartPanelProps) {
  const [localTf, setLocalTf] = useState(initialTimeframe);
  const rows = Math.ceil(charts.length / cols);
  const subHeight = height ? Math.max(150, Math.floor(height / rows) - 100) : 150;
  ...
  const handleResetZoom = useCallback((e: React.MouseEvent, chartTitle?: string) => {
    e.stopPropagation();
    if (chartTitle) {
  ...
  const handleExpand = () => {
    if (onClick) {
        ...
        const zoomedTitle = Object.keys(zoomedCharts).find(k => zoomedCharts[k]);
            ...
            {charts.map((chart) => (
                ...
                >
                    <Group justify="space-between" mb={4}>
                        <Text size="xs" c="dimmed">{chart.title}</Text>
                    ...
                    <div className={classes.flexGrow}>
                        <BaseChart 
                            data={chart.data} 
                            series={[{ name: chart.seriesName || 'Value', color: chart.color || '#4dabf7', unit: chart.unit }]} 
                            height={subHeight} 
                            timeframe={localTf}
                            tooltipFormat={tooltipFormat}
                            onCreate={(u) => uplotRefs.current[chart.title] = u}
                            onZoom={(isZoomed) => setZoomedCharts(prev => ({ ...prev, [chart.title]: isZoomed }))}
